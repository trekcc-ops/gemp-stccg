...
public class EffectBlueprintDeserializer {
    ...
    public static List<EffectBlueprint> getEffectBlueprints(JsonNode node)
            throws InvalidCardDefinitionException {
        List<EffectBlueprint> result = new LinkedList<>();
        for (JsonNode effect : JsonUtils.toArray(node)) {
            result.add(getEffectBlueprint(effect));
        }
        return result;
    }
}