...
public class AddShieldsModifierBlueprint implements ModifierBlueprint {

    private final FilterBlueprint _modifiedCardFilterBlueprint;
    private final List<Requirement> _requirements = new ArrayList<>();
    private final SingleValueSource _amount;
    private final boolean _isCumulative;

    AddShieldsModifierBlueprint(@JsonProperty(value = "modifiedCards", required = true)
                              FilterBlueprint modifiedCardFilterBlueprint,
                                @JsonProperty(value = "amount", required = true)
                                SingleValueSource amount,
    ...
    public Modifier createModifier(DefaultGame cardGame, PhysicalCard thisCard, GameTextContext actionContext) {
        ...
        Evaluator modifierAmount = _amount.getEvaluator(cardGame, actionContext);
        return new ShieldsModifier(thisCard, affectFilter, ifCondition, modifierAmount, _isCumulative);
    }
