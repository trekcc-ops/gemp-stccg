...
public class ActionSourceAppender {
    public void processEffect(JsonNode node, CardBlueprint blueprint)
        ...
        final Phase phase =
                BlueprintUtils.getEnum(Phase.class, BlueprintUtils.getString(node, "phase"), "phase");
        ...
        if (!isResponse) {
            ...
            for (JsonNode trigger : triggers) {
                TriggerChecker triggerChecker = new TriggerCheckerFactory().getTriggerChecker(trigger);
