...
public class WalkCardsAction extends BeamOrWalkAction {
    ...
    public WalkCardsAction(DefaultGame cardGame, Player player, CardWithCrew cardWalkingFrom) {
    ...
    protected Collection<PhysicalCard> getDestinationOptions(ST1EGame game) {
        ...
        } else if (_cardSource instanceof FacilityCard facility) {
            result.addAll(facility.getDockedShips());
        }
        return result;
    }
    ...
    public boolean requirementsAreMet(DefaultGame cardGame) {
        return cardGame instanceof ST1EGame stGame &&
                !getDestinationOptions(stGame).isEmpty() &&
                !_cardSource.getPersonnelAboard(cardGame).isEmpty();
    }
