...
public class CheckTurnLimitEffect extends UnrespondableEffect {
    ...
    public CheckTurnLimitEffect(DefaultGame game, Action action, PhysicalCard card, int limit, Effect limitedEffect) {
        _card = card;
        _limit = limit;
    ...
    public void doPlayEffect() {
        int incrementedBy = _game.getModifiersQuerying().getUntilEndOfTurnLimitCounter(_card).incrementToLimit(_limit, 1);
        if (incrementedBy > 0) {
