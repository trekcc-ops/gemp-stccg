        ...
        actionsEnvironment.addAlwaysOnActionProxy(
                new AbstractActionProxy() {
                    ...
                    public List<? extends Action> getOptionalAfterActions(String playerId, DefaultGame game, EffectResult effectResult) {
                        ...
                        for (PhysicalCard responseEvent : Filters.filter(game.getGameState().getHand(playerId), game, CardType.EVENT, Keyword.RESPONSE)) {
                    ...
                    @Override
                    public List<? extends Action> getOptionalBeforeActions(String playerId, DefaultGame game,
                                                                           Effect effect) {
                        List<Action> result = new LinkedList<>();
                        for (PhysicalCard responseEvent : Filters.filter(game.getGameState().getHand(playerId), game,
                                CardType.EVENT, Keyword.RESPONSE)) {
                            if (LotroPlayUtils.checkPlayRequirements(game, responseEvent, Filters.any,
                                    0, 0, false, false,
                                    false)) {
                                final List<PlayEventAction> actions = responseEvent.getBlueprint()
                                        .getPlayResponseEventBeforeActions(playerId, game, effect, responseEvent);
                                if (actions != null)
                                    result.addAll(actions);
                            }
                        }
